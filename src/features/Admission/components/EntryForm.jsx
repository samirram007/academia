...
const EntryForm = ({ initialValues, entryMode }) => {
    ...
    const admissionStoreMutation = useStoreAdmissionMutation()
    const admissionUpdateMutation = useUpdateAdmissionMutation()
    const admissionDeleteMutation = useDeleteAdmissionMutation()

    const handleFormSubmit = (values) => {
        if (entryMode === 'create') {
            admissionStoreMutation.mutate(values)
        } else if (entryMode === 'edit') {

            admissionUpdateMutation.mutate(values)

        } else if (entryMode === 'delete') {

            admissionDeleteMutation.mutate(values)
            ...
            <form onSubmit={formik.handleSubmit}>
                <div className='grid grid-cols-1  '>
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-5'>
                        ...
                        <div>
                            <FormikInputBox formik={formik} name="name" label="Admission" />
