...
import {
    ...
    useStoreTransportFeeMutation,
    useUpdateTransportFeeMutation
...
import { useTransportUserSearch, useUserSearch } from '../../TransportUser/hooks/quaries';
        ...
        <div className='relative h-[91vh] w-full'>
            {
                isSearching ?
                    ...
                    <>
                        <SelectedUserPanel selectedTransportUser={selectedTransportUser}
    ...
    return (
        <div className='flex flex-row justify-between items-center bottom-2 border-blue-400/30 border-2 rounded-xl p-2'>
            <div className='flex flex-col gap-2'>
                ...
                <div className='flex flex-row items-center gap-4'> <span className='font-bold text-lg'>{user.name} </span><span className='badge bg-yellow-400 text-slate-800 uppercase'> {user.user_type}</span></div>
                ...
                <div>Monthly Charge: {data.is_free ? 'Free' : `Rs.${data.monthly_charge}/-`}</div>
            </div>
            <div><button onClick={() => handleUserSelection(user.id)} className='btn btn-sm btn-primary btn-rounded'>Fees</button></div>
        </div>
    )
}
const SelectedUserPanel = ({ selectedTransportUser: user, setSearching, entryMode }) => {
    const switchUser = () => {
        setSearching(prev => !prev)
    }
    return (
        <div className='flex flex-row justify-between items-center bottom-2 border-blue-400/30 border-2 rounded-xl p-2 m-2'>
            <div>
            <span className='text-green-600 font-bold'>{Capitalize(user.user.user_type)}</span>: {user.user.name}
