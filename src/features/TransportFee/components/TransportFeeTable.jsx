...
export default function TransportFeeTable({ data, columns, pageSize = 100, createRoute,
    ...
    mobileHeaders = ['id', 'name'], TransportFeeData, initialFilterValues }) {
    ...
    if (TransportFeeData.isFetching) return <div>Loading</div>
        ...
        <div className='container-flex md-container'>
            ...
            {/* {
                ...
                <div>
                    ...
                    {
                        (!TransportFeeData.data || TransportFeeData.isError || TransportFeeData.data.data.length === 0)
        ...
        <div className='flex w-100 flex-row flex-nowrap bg-blue-600/10 p-2 rounded-lg gap-2'>
            <div className='flex flex-col gap-2 w-1/2'>
                ...
                <div>
                    ...
                    <div className='px-2'>
                        <span className='text-green-500'>{Capitalize(data.user.user_type)}: </span><span className="font-bold text-green-500">{data.user.name}</span>
                    </div>
                    <div className='px-2'>
                        <span className='w-16'>Campus: </span><span>{data.campus.name}</span>
                    </div>
                    <div className='px-2'>
                        <span className='w-16'>Session: </span><span>{data.academic_session.session}</span>
                    </div>
...
export const TransportFeeDetails = ({ data }) => {
    ...
    return (
        ...
        gap-1   border-b mb-1 bg-slate-400 text-slate-800 font-bold text-[8px]'>
            ...
            {data && data.transport_fee_items.map((item, index) => (
                <div key={index} className='text-xs/[12px] p-1 flex flex-row flex-nowrap justify-between items-center
                    ...
                    <div className='flex flex-row gap-2 '>
                        {item.fee_head.name}
                        {
                            item.transport_fee_item_months &&
                            item.transport_fee_item_months.length > 0 &&
                            item.transport_fee_item_months.map((x, index) => (
...
export const TransportFeeItems = ({ fees, session_id }) => {
